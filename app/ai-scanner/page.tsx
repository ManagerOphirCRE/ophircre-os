...
export default function AIScannerPage() {
  ...
  async function saveLeaseToDatabase() {
    ...
    try {
      ...
      const { error: lErr } = await supabase.from('leases').insert([{ 
        ...
        start_date: extractedLease.start_date || null, 
        end_date: extractedLease.end_date || null, 
        base_rent_amount: extractedLease.base_rent ? Number(extractedLease.base_rent) : null 
      }]);
      if (lErr) throw lErr;
      
      alert("Tenant and Lease saved!"); 
      setExtractedLease(null); 
      setLeaseFile(null);
    } catch (error: any) { 
  ...
  // Helper to handle file selection safely
  ...
  return (
    <>
      ...
      <main className="flex-1 overflow-y-auto p-8 flex flex-col md:flex-row md:space-x-6 space-y-6 md:space-y-0 bg-gray-100">
        ...
        {/* LEFT COLUMN: Upload */}
        <div className="w-full md:w-1/2 bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-fit md:h-[calc(100vh-120px)]">
          <h3 className="font-bold text-gray-800 mb-2">1. Upload {activeTab === 'lease' ? 'Lease' : 'Statement'}</h3>
          <p className="text-sm text-gray-500 mb-4">Select a file. The AI will extract the data automatically.</p>
          
          <div className="flex-1 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center bg-gray-50 p-6 min-h-[200px]">
            <input 
              type="file" 
              accept={activeTab === 'lease' ? ".pdf" : ".pdf,image/png,image/jpeg,image/jpg,image/*"} 
              onChange={handleFileChange} 
              className="mb-4 w-full max-w-xs" 
            />
            
            {/* THIS IS THE GREEN TEXT THAT WAS MISSING! */}
            {activeTab === 'lease' && leaseFile && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg w-full text-center">
                <p className="text-green-700 font-bold text-sm">✓ Selected: {leaseFile.name}</p>
              </div>
            )}
            {activeTab === 'invoice' && invoiceFile && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg w-full text-center">
                <p className="text-green-700 font-bold text-sm">✓ Selected: {invoiceFile.name}</p>
              </div>
            )}
          </div>
          
          {/* THIS IS THE BIG PURPLE BUTTON! */}
          <button 
            ...
            className={`w-full mt-6 py-4 rounded-lg font-black text-white transition shadow-md text-lg ${(activeTab === 'lease' ? (isScanningLease || !leaseFile) : (isScanningInvoice || !invoiceFile)) ? 'bg-purple-300 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 hover:-translate-y-1'}`}
          >
            {activeTab === 'lease' ? (isScanningLease ? '🤖 Reading Lease...' : '✨ Scan Lease with AI') : (isScanningInvoice ? '🤖 Reading Invoice...' : '✨ Scan Invoice with AI')}
          </button>
        </div>

        {/* RIGHT COLUMN: Results */}
        <div className="w-full md:w-1/2 bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-fit">
