...
export default function FinancialsPage() {
  ...
  async function syncBanks() {
    ...
    try {
      ...
      fetchData();
    } catch (e: any) { alert("Sync Error: " + e.message + "\n(Note: Add PLAID_CLIENT_ID and PLAID_SECRET to Vercel)"); } 
    ...
    <>
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center">
        ...
        <div className="space-x-3 flex items-center">
          ...
          <button onClick={syncBanks} disabled={isSyncing} className={`px-4 py-2 rounded-md font-medium text-white transition shadow-sm ${isSyncing ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'}`}>
            {isSyncing ? 'Syncing...' : '🔄 Sync Banks'}
          </button>

          <button onClick={() => openPlaid()} disabled={!plaidReady} className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-md font-medium transition shadow-sm flex items-center">
