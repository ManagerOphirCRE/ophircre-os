...
export default function FinancialsPage() {
  ...
  const [tenantLeases, setTenantLeases] = useState<any[]>([]) // UPGRADE: Holds lease data for auto-splits
  ...
  async function fetchData() {
    ...
    // UPGRADE: Fetch leases with tenant names for the Auto-Split Engine
    const { data: leases } = await supabase.from('leases').select('*, tenants(name)')
    if (leases) setTenantLeases(leases)
  }

  // --- THE AUTO-SPLIT ENGINE ---
  // This runs every time you type in the Description box
  useEffect(() => {
    ...
    if (matchedLease) {
      ...
      // Auto-generate the splits based on their lease!
      const autoSplits = []
      if (matchedLease.base_rent_amount > 0) autoSplits.push({ account_id: rentAcc, property_id: '', amount: matchedLease.base_rent_amount, memo: 'Base Rent' })
      if (matchedLease.cam_charge > 0) autoSplits.push({ account_id: camAcc, property_id: '', amount: matchedLease.cam_charge, memo: 'CAM Escrow' })
      if (matchedLease.tax_charge > 0) autoSplits.push({ account_id: camAcc, property_id: '', amount: matchedLease.tax_charge, memo: 'Tax Escrow' })
      if (matchedLease.insurance_charge > 0) autoSplits.push({ account_id: camAcc, property_id: '', amount: matchedLease.insurance_charge, memo: 'Insurance Escrow' })
      
      if (autoSplits.length > 0) {
        setSplits(autoSplits)
        // Auto-calculate total amount
        const total = autoSplits.reduce((sum, s) => sum + Number(s.amount), 0)
        setTotalAmount(total.toString())
      }
    }
  }, [description]) // Listens to changes in description
  ...
  function updateSplit(index: number, field: string, value: string) {
    const newSplits = [...splits]; newSplits[index] = { ...newSplits[index], [field]: value }; setSplits(newSplits)
  }
  
  async function handleAccountSelect(index: number, value: string) {
    if (value === 'NEW_CATEGORY') {
      const newName = prompt("Enter new category name:")
      const type = prompt("Enter account type (Asset, Liability, Equity, Revenue, Expense):", "Expense")
      if (!newName || !type) return
      const { data } = await supabase.from('chart_of_accounts').insert([{ name: newName, account_type: type }]).select().single()
      if (data) { await fetchData(); updateSplit(index, 'account_id', data.id) }
    } else {
      updateSplit(index, 'account_id', value)
    }
  }
  ...
  async function saveTransaction() {
    ...
    const journalEntries = splits.map(split => ({
      transaction_id: txnData?.id, account_id: split.account_id, property_id: split.property_id || null, description: split.memo || description, debit: Number(split.amount), credit: 0
    }))
  ...
  async function handleBulkFileUpload(event: any) {
    ...
    pastEntries?.forEach(entry => {
      const txnDesc = (entry.transactions as any)?.description?.toLowerCase()
      const jeDesc = entry.description?.toLowerCase()
      const desc = txnDesc || jeDesc
    ...
    for (const file of files as any[]) {
      ...
      const mappedRows = parsedData.map((row: any) => {
        ...
        return { date: row.Date || row.date || '', description: desc, amount: amount, account_id: memoryMap[desc.toLowerCase()] || '', property_id: '', source_file: file.name }
      })
      allCombinedRows = [...allCombinedRows, ...mappedRows]
    }

    setCsvRows(allCombinedRows)
    setIsProcessingCsv(false)
    setIsCsvModalOpen(true)
  }

  function updateCsvRow(index: number, field: string, value: string) {
    const newRows = [...csvRows]; newRows[index] = { ...newRows[index], [field]: value }; setCsvRows(newRows)
  }

  async function saveCsvTransactions() {
    for (const row of csvRows) {
      if (!row.account_id) continue 
      const { data: txnData } = await supabase.from('transactions').insert([{ date: row.date, description: row.description, total_amount: row.amount, status: 'Approved' }]).select().single()
      await supabase.from('journal_entries').insert([{ transaction_id: txnData?.id, account_id: row.account_id, property_id: row.property_id || null, description: `Imported from ${row.source_file}`, debit: row.amount, credit: 0 }])
    }
      ...
      <main className="flex-1 overflow-y-auto p-8 relative">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {transactions.map((txn) => (
                <tr key={txn.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{txn.description}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">${txn.total_amount}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {isCsvModalOpen && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-xl shadow-lg w-[1100px] max-h-[90vh] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto mb-6 border rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Bank File</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {csvRows.map((row, index) => (
                      <tr key={index} className={row.account_id ? 'bg-white' : 'bg-red-50'}>
                        <td className="px-4 py-2 text-xs text-gray-400">{row.source_file}</td>
                        <td className="px-4 py-2 text-sm">{row.date}</td>
                        <td className="px-4 py-2 text-sm font-medium">{row.description}</td>
                        <td className="px-4 py-2 text-sm">${row.amount}</td>
                        <td className="px-4 py-2">
                          <select className="border p-1 rounded text-sm w-full" value={row.account_id} onChange={(e) => updateCsvRow(index, 'account_id', e.target.value)}>
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t">
                <button onClick={() => setIsCsvModalOpen(false)} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded font-medium">Cancel</button>
                <button onClick={saveCsvTransactions} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded font-medium">Approve & Import All</button>
              </div>
            </div>
          </div>
        )}
        ...
        {isModalOpen && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-xl shadow-lg w-[1000px] max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {/* UPGRADE: Typing in here triggers the Auto-Split Engine! */}
                <div><label className="block text-sm font-medium mb-1 text-blue-600">Payee / Description (Auto-Splits)</label><input type="text" placeholder="Type a tenant name..." className="w-full border-2 border-blue-200 p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none" value={description} onChange={(e) => setDescription(e.target.value)} /></div>
              ...
              <div className="mb-4 flex justify-between items-end">
                <h4 className="font-semibold text-gray-800">Categorize (Splits)</h4>
                <button onClick={addSplitRow} className="text-sm text-blue-600 font-medium">+ Add Split Line</button>
              </div>
              <div className="space-y-3 mb-6">
                {splits.map((split, index) => (
                  <div key={index} className="flex space-x-2 items-center">
                    <select className="w-1/4 border p-2 rounded text-sm" value={split.account_id} onChange={(e) => handleAccountSelect(index, e.target.value)}>
                      <option value="">Select Category...</option>
                    ...
                    <select className="w-1/4 border p-2 rounded text-sm" value={split.property_id} onChange={(e) => updateSplit(index, 'property_id', e.target.value)}>
                      <option value="">Assign Property (Opt)</option>
              ...
              <div className="flex justify-between items-center border-t pt-4 mt-6">
                ...
                <div className="space-x-3">
                  ...
                  <button onClick={saveTransaction} disabled={!isBalanced} className={`px-4 py-2 rounded text-white ${isBalanced ? 'bg-blue-600' : 'bg-gray-400'}`}>Save Transaction</button>
                </div>
              </div>
            </div>
          </div>
