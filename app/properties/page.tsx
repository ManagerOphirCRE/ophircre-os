...
export default function PropertiesPage() {
  ...
  async function saveProperty() {
    await supabase.from('properties').insert([{ name: newProperty.name, address: newProperty.address, total_sqft: Number(newProperty.total_sqft) }])
  ...
  return (
    <>
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md font-medium transition shadow-sm">+ Add Property</button>
      </header>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {properties.map((property) => (
                <tr 
                  key={property.id} 
                  // UPGRADE: Instant client-side routing instead of hard reload
