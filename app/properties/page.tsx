...
export default function PropertiesPage() {
  ...
  async function saveProperty() {
    ...
    setNewProperty({ name: '', address: '', total_sqft: '' }) // Clear form
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr key={property.id} onClick={() => window.location.href = `/properties/${property.id}`} className="hover:bg-blue-50 cursor-pointer transition">
        ...
        {isModalOpen && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-8 rounded-xl shadow-lg w-96">
              ...
              <input 
                type="text" placeholder="Property Name" className="w-full border p-2 mb-3 rounded"
                value={newProperty.name} onChange={(e) => setNewProperty({...newProperty, name: e.target.value})}
              />
              <input 
                type="text" placeholder="Address" className="w-full border p-2 mb-3 rounded"
                value={newProperty.address} onChange={(e) => setNewProperty({...newProperty, address: e.target.value})}
              />
              <input 
                type="number" placeholder="Total Square Footage" className="w-full border p-2 mb-4 rounded"
              ...
              <div className="flex justify-end space-x-2">
                ...
                <button onClick={saveProperty} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Save</button>
              </div>
            </div>
          </div>
        )}
