  ...
  useEffect(() => {
    async function loadSecureTenant() {
      ...
      if (session?.user?.email) {
        const { data } = await supabase.from('tenants').select('*, leases(*, spaces(name, properties(name)))').ilike('contact_email', session.user.email).single()
  ...
  async function submitTicket(e: any) {
    ...
    try {
      ...
      if (ticketFile) {
        ...
        const { error: uploadError } = await supabase.storage.from('documents').upload(fileName, ticketFile)
