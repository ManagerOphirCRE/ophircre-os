...
export default function LeasingPipelinePage() {
  ...
  const [brokers, setBrokers] = useState<any[]>([]); // NEW: Brokers list
  ...
  const [selectedBroker, setSelectedBroker] = useState(''); // NEW: Selected Broker

  useEffect(() => { 
    fetchProspects(); fetchProperties(); fetchBrokers();
  },[]);
  ...
  function openApproveModal(prospect: any) {
    setApprovingProspect(prospect); setIsModalOpen(true);
  }
  ...
  async function finalizeLease(e: any) {
    ...
    try {
      ...
      // NEW: COMMISSION HUMAN-IN-THE-LOOP
      if (selectedBroker) {
        ...
        await supabase.from('tasks').insert([{
          title: `APPROVE COMMISSION: ${broker.name}`,
          description: `Lease signed for ${approvingProspect.name}. Estimated commission is $${estCommission.toFixed(2)} (${broker.commission_rate}% of total lease value).\n\nPlease review this amount and add it to Accounts Payable when approved.`,
          status: 'To Do'
        }]);
      }

      alert("Prospect approved! Commission approval task created (if broker was selected).");
      setIsModalOpen(false); fetchProspects();
  ...
  return (
    <main className="flex-1 overflow-y-auto p-8 bg-gray-100 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
        ...
        <div className="bg-blue-50 rounded-xl p-4 border border-blue-100"><h3 className="font-bold text-blue-800 mb-4">Under Review ({reviewing.length})</h3><div className="space-y-3">{reviewing.map(p => (<div key={p.id} className="bg-white p-4 rounded-lg shadow-sm border border-blue-200"><p className="font-bold text-blue-600">{p.name}</p><div className="flex justify-between mt-3"><button onClick={() => moveProspect(p.id, 'prospect_new')} className="text-xs text-gray-500">← Back</button><button onClick={() => moveProspect(p.id, 'prospect_negotiation')} className="text-xs text-blue-600 font-bold">Negotiate →</button></div></div>))}</div></div>
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg w-[500px]">
            ...
            <form onSubmit={finalizeLease} className="space-y-4 mt-4">
              ...
              <div className="pt-2 border-t">
                <label className="block text-xs font-bold text-orange-600 mb-1">Procuring Broker (Optional)</label>
                <select className="w-full border p-2 rounded outline-none" value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
