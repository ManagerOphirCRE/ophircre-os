...
export default function TenantProfilePage() {
  ...
  // NEW: Manual Escalation Execution
  async function executeEscalation() {
    if (!confirm("Execute this escalation? This will permanently update the base rent and email the tenant an official notice.")) return;
    setIsExecuting(true);
    try {
      ...
      if (tenant.contact_email) {
        await fetch('/api/send-email', {
          ...
          body: JSON.stringify({
            ...
            text: `Hello ${tenant.name},\n\nThis is an official notice regarding your lease.\n\nPer the terms of your agreement, your monthly Base Rent has escalated.\n\nYour new Base Rent amount is $${newRent.toFixed(2)}.\n\nThis new amount will be reflected on your next invoice. You can view your ledger at any time in your secure portal: https://app.ophircre.com/portal-login\n\nThank you,\nOphirCRE Management`
          })
        });
        
        // Log communication
        await supabase.from('communications').insert([{ tenant_id: tenant.id, subject: 'Official Notice: Rent Escalation', body: `Rent increased to $${newRent.toFixed(2)}`, type: 'Email', status: 'Sent' }]);
      }

      alert(`Escalation Executed! Rent increased to $${newRent.toFixed(2)}.`);
      setBaseRent(newRent);
      setEscalationDate('');
    } catch (error: any) {
              ...
              {lease ? (
                <div className="space-y-6 max-w-md">
                  ...
                  <div className="space-y-3 pt-4 border-t">
                    ...
                    {escalationDate && (
                      <button onClick={executeEscalation} disabled={isExecuting} className="w-full mt-2 py-2 rounded font-bold text-white bg-orange-500 hover:bg-orange-600 transition shadow-sm">
                        {isExecuting ? 'Executing...' : '⚡ Execute Escalation Now'}
                      </button>
                    )}
                  </div>
