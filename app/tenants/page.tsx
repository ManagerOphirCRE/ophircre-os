              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {tenants.map((tenant) => (
                  <tr key={tenant.id} onClick={() => router.push(`/tenants/${tenant.id}`)} className="hover:bg-blue-50 cursor-pointer transition">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{tenant.contact_phone || '-'}</td>
                    <td className="px-6 py-4 text-right">
                      <button onClick={(e) => deleteTenant(e, tenant.id)} className="text-xs bg-red-100 text-red-600 px-3 py-1 rounded font-bold hover:bg-red-200 transition">Trash</button>
                    </td>
                  </tr>
                ))}
