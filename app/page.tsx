...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      // 4. Build Financial Chart Data (Revenue vs Expenses)
      const { data: journalEntries } = await supabase
        .from('journal_entries')
        .select('debit, credit, chart_of_accounts(account_type), transactions(date)')
    ...
    <main className="flex-1 overflow-y-auto p-8 bg-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="md:col-span-1 bg-white rounded-xl shadow-sm border border-gray-200 p-6 overflow-y-auto h-96">
          ...
          <div className="space-y-3">
            ...
            {recentTasks.length === 0 && <p className="text-sm text-gray-500 text-center mt-10">No open tasks.</p>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {surveys.length === 0 && <p className="text-sm text-gray-500 col-span-3 text-center py-4">No surveys submitted yet.</p>}
        </div>
      </div>
