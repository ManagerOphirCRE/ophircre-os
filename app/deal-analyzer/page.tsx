...
export default function DealAnalyzerPage() {
  ...
  async function acquireDeal(deal: any) {
    if (!confirm(`Acquire ${deal.property_name}? This moves it to your active Portfolio.`)) return;
    try {
      ...
      const { data } = await supabase.from('deals').select('*').order('created_at', { ascending: false });
      if (data) setDeals(data);
    } catch (error: any) { alert("Error acquiring asset: " + error.message); }
  }
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-8 bg-gray-100">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8 flex flex-col md:flex-row items-center justify-between">
          <div className="mb-4 md:mb-0"><h3 className="font-bold text-gray-800">Upload Offering Memorandum (OM)</h3><p className="text-sm text-gray-500">Upload a broker PDF or pro-forma image. The AI will extract the financials.</p></div>
          <div className="flex items-center space-x-4 w-full md:w-auto">
            ...
            <button onClick={handleScanOM} disabled={isScanning || !omFile} className={`px-6 py-2 rounded-md font-bold text-white transition shadow-sm whitespace-nowrap ${isScanning || !omFile ? 'bg-purple-400' : 'bg-purple-600 hover:bg-purple-700'}`}>{isScanning ? '🤖 Analyzing...' : '✨ Analyze OM'}</button>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-xl shadow-sm p-6 mb-8 flex space-x-8 text-white">
          <div>
            ...
            <div className="flex space-x-4">
              <div><label className="block text-xs text-slate-400 mb-1">Down Payment (%)</label><input type="number" className="bg-slate-800 border border-slate-700 p-2 rounded text-white outline-none w-24" value={downPaymentPct} onChange={(e) => setDownPaymentPct(Number(e.target.value))} /></div>
              <div><label className="block text-xs text-slate-400 mb-1">Interest Rate (%)</label><input type="number" step="0.1" className="bg-slate-800 border border-slate-700 p-2 rounded text-white outline-none w-24" value={interestRate} onChange={(e) => setInterestRate(Number(e.target.value))} /></div>
            </div>
          </div>
        </div>
            ...
            return (
              <div key={deal.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
                ...
                <div className="p-6 flex-1 space-y-4">
                  ...
                  <div className="flex justify-between items-end border-b pb-2"><span className="text-sm text-gray-500 font-medium">NOI</span><span className="text-lg font-bold text-green-600">${noi.toLocaleString()}</span></div>
                  <div className="flex justify-between items-end border-b pb-2"><span className="text-sm text-gray-500 font-medium">Cap Rate</span><span className={`text-lg font-bold ${isGoodCap ? 'text-green-600' : 'text-orange-500'}`}>{deal.cap_rate}%</span></div>
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-200 flex space-x-2">
                  {/* NEW: Print Pro-Forma Button */}
                  <button onClick={() => printProForma(deal, downPaymentAmount, loanAmount, annualDebtService, netCashFlow, cashOnCash)} className="bg-gray-800 hover:bg-black text-white px-3 py-2 rounded font-bold transition shadow-sm">🖨️ PDF</button>
