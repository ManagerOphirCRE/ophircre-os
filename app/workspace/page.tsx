...
export default function WorkspacePage() {
  ...
  async function loadLocalDataAndSync() {
    ...
    } catch (e: any) { setSyncError(e.message); setIsLoading(false); }
  }
  ...
  async function handleEmailAction(action: string) {
    ...
    try {
      ...
      } else { alert("Email marked as read in Gmail!"); }
    } catch (error: any) { alert("Action Error: " + error.message); } finally { setIsActioning(false); }
  }
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-[calc(100vh-150px)]">
            <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center rounded-t-xl">
              <div className="flex items-center space-x-4">
                ...
                <select className="border p-1 rounded text-sm outline-none bg-white font-bold text-blue-600" value={selectedAccount} onChange={(e) => setSelectedAccount(e.target.value)}><option value="ALL">All Connected Accounts</option>{accounts.map(acc => <option key={acc} value={acc}>{acc}</option>)}</select>
                <select className="border p-1 rounded text-sm outline-none bg-white text-gray-600" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)}><option value="newest">Sort: Newest First</option><option value="oldest">Sort: Oldest First</option><option value="sender">Sort: By Sender</option></select>
              </div>
              ...
              {filteredEmails.map((email) => (
                <div key={email.id} onClick={() => setViewingEmail(email)} className="p-4 border-b border-gray-100 hover:bg-blue-50 transition flex flex-col group cursor-pointer">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className="text-xs text-gray-500 font-medium whitespace-nowrap">{new Date(email.date).toLocaleString([], {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'})}</span>
                  </div>
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-40 p-4">
            <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
              <div className="p-6 border-b flex justify-between items-start bg-gray-50">
                <div><h3 className="text-xl font-bold text-gray-900 mb-2">{viewingEmail.subject}</h3><p className="text-sm text-gray-600"><strong>From:</strong> {viewingEmail.sender}</p><p className="text-sm text-gray-600"><strong>To:</strong> {viewingEmail.account_email}</p><p className="text-xs text-gray-400 mt-1">{new Date(viewingEmail.date).toLocaleString()}</p></div>
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-2xl">
              ...
              <form onSubmit={sendGmail} className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t"><button type="button" onClick={() => setIsComposing(false)} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded font-medium">Cancel</button><button type="submit" disabled={isSending} className={`px-6 py-2 rounded font-bold text-white transition shadow-sm ${isSending ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'}`}>{isSending ? 'Sending...' : 'Send Email'}</button></div>
              </form>
            </div>
          </div>
        )}
