...
export default function LeaseDrafterPage() {
  ...
  // NEW: Send to Portal for E-Signature
  async function sendForSignature() {
    if (!selectedTenant || !generatedLease) return alert("Please select a tenant and generate the document first.");
    setIsSending(true);
    try {
      // Find the lease record we created in the Leasing Pipeline
      const { data: lease } = await supabase.from('leases').select('id').eq('tenant_id', selectedTenant.id).single();
      if (!lease) throw new Error("No lease found. Please approve this prospect in the Leasing Pipeline first to generate their lease record.");

      // Save the HTML and update status
      const { error } = await supabase.from('leases').update({ 
        document_html: generatedLease, 
      ...
      if (selectedTenant.contact_email) {
        await fetch('/api/send-email', {
          ...
          body: JSON.stringify({
            ...
            subject: "ACTION REQUIRED: Review and Sign Your Lease",
            text: `Hello ${selectedTenant.name},\n\nYour lease agreement is ready for review and signature. Please log into your secure portal to execute the document:\nhttps://app.ophircre.com/portal-login\n\nThank you,\nOphirCRE Management`
          })
        });
      }

      alert("Lease successfully sent to the Tenant Portal for E-Signature!");
    } catch (error: any) {
    ...
    <>
      ...
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center print:hidden">
        ...
        <div className="space-x-3">
          ...
          <button onClick={sendForSignature} disabled={isSending} className={`px-4 py-2 rounded-md font-medium text-white transition shadow-sm ${isSending ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'}`}>
            {isSending ? 'Sending...' : '✍️ Send for E-Signature'}
          </button>
        </div>
      </header>
