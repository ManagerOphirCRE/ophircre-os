...
export default function TasksPage() {
  ...
  async function fetchData() {
    const { data: tData } = await supabase.from('tasks').select('*, tenants(name, contact_phone), properties(name), vendors(company_name)').order('created_at', { ascending: false });
  ...
  function openTask(task: any) {
    setSelectedTask(task); setChatHistory(task.comments ||[]); setVideoUrl(null); fetchBids(task.id);
  }

  async function addTask() {
    if (!newTaskTitle) return;
    await supabase.from('tasks').insert([{ title: newTaskTitle, status: 'To Do', organization_id: orgId }]);
  ...
  // NEW: Broadcast to Trade (Uber Style)
  async function broadcastToTrade() {
    const trade = prompt("Enter the trade to broadcast to (e.g., Plumbing, Electrical):");
    if (!trade) return;
    try {
      await supabase.from('tasks').update({ broadcast_trade: trade }).eq('id', selectedTask.id);
      alert(`Job broadcasted to all ${trade} vendors! The first to claim it in their portal gets the job.`);
      fetchData(); setSelectedTask(null);
  ...
  async function requestBid() {
    ...
    try {
      await supabase.from('task_bids').insert([{ task_id: selectedTask.id, vendor_id: selectedVendorId, status: 'Pending', organization_id: orgId }]);
      alert("Bid requested!"); fetchBids(selectedTask.id);
    } catch (error: any) { alert("Error: " + error.message); }
  }

  async function awardJob(bidId: string) {
    if (!confirm("Award the job?")) return;
    try {
  ...
  async function sendChatMessage(e: any) {
    e.preventDefault(); if (!chatMessage) return;
    const newMessage = { sender: 'Management', text: chatMessage, timestamp: new Date().toISOString() };
    const updatedHistory = [...chatHistory, newMessage];
    try {
      await supabase.from('tasks').update({ comments: updatedHistory }).eq('id', selectedTask.id);
      setChatHistory(updatedHistory); setChatMessage(''); fetchData();
    } catch (error: any) { alert("Error: " + error.message); }
  }
  ...
  async function startVideoCall() {
    ...
    try {
      const res = await fetch('/api/create-video-room', { method: 'POST' });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setVideoUrl(data.url);
      if (selectedTask.tenants?.contact_phone) {
        const phone = selectedTask.tenants.contact_phone;
        const formattedPhone = phone.startsWith('+') ? phone : `+1${phone.replace(/\D/g,'')}`;
        await fetch('/api/send-sms', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ to: formattedPhone, body: `OphirCRE Video Call: Join management here: ${data.url}` }) });
        alert("Video room created and SMS sent!");
      }
    } catch (error: any) { alert("Video Error: " + error.message); } finally { setIsStartingVideo(false); }
  }
            ...
            <div className="space-y-3">
              {todoTasks.map(task => (
                <div key={task.id} onClick={() => openTask(task)} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 cursor-pointer hover:border-blue-500 transition">
                  ...
                  {task.broadcast_trade && <p className="text-xs text-orange-600 mt-1 font-bold">📡 Broadcasted: {task.broadcast_trade}</p>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {inProgressTasks.map(task => (
                <div key={task.id} onClick={() => openTask(task)} className="bg-white p-4 rounded-lg shadow-sm border border-blue-200 cursor-pointer hover:border-blue-500 transition">
                  <p className="font-medium text-gray-800">{task.title}</p>
                  {task.vendors && <p className="text-xs text-green-600 mt-1 font-bold">Claimed by: {task.vendors.company_name}</p>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-5xl max-h-[90vh] flex flex-col">
              <div className="flex justify-between items-start mb-4 border-b pb-4">
                ...
                <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded text-xs font-bold uppercase tracking-wider">{selectedTask.status}</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto mb-6 flex flex-col md:flex-row md:space-x-8 space-y-6 md:space-y-0">
                <div className="w-full md:w-1/2 space-y-4 text-sm pr-2">
                  ...
                  <div><strong className="text-gray-700">Description / Notes:</strong><p className="mt-2 p-4 bg-gray-50 rounded-lg border border-gray-200 text-gray-800 whitespace-pre-wrap font-mono text-xs">{selectedTask.description || 'No description provided.'}</p></div>

                  <div className="bg-orange-50 p-4 rounded-lg border border-orange-200 mt-4">
                    <strong className="text-orange-800 block mb-2">Vendor Dispatch & Bidding</strong>
                    ...
                    <button onClick={broadcastToTrade} className="w-full bg-slate-900 hover:bg-black text-white px-4 py-2 rounded font-bold transition shadow-sm mb-4">📡 Broadcast to All Vendors (Uber Style)</button>
                      ...
                      <div className="space-y-2">
                        ...
                        {taskBids.map(bid => (
                          <div key={bid.id} className="bg-white p-3 rounded border flex justify-between items-center">
                            <div><p className="font-bold text-gray-800">{bid.vendors?.company_name}</p><p className="text-xs text-gray-500">{bid.notes || 'No notes.'}</p></div>
                            <div className="text-right flex items-center space-x-4">
                              <span className="font-black text-green-700">${Number(bid.bid_amount).toLocaleString()}</span>
                              {bid.status === 'Pending' && <button onClick={() => awardJob(bid.id)} className="bg-green-600 text-white px-3 py-1 rounded text-xs font-bold">Award Job</button>}
                ...
                <div className="w-full md:w-1/2 flex flex-col border-l pl-8">
                  <div className="flex justify-between items-center mb-2">
                    <strong className="text-gray-700">Live Chat with Tenant</strong>
                    {!videoUrl ? <button onClick={startVideoCall} disabled={isStartingVideo} className="text-xs bg-purple-600 hover:bg-purple-700 text-white px-3 py-1 rounded font-bold transition shadow-sm">{isStartingVideo ? 'Starting...' : '📹 Start Video Call'}</button> : <button onClick={() => setVideoUrl(null)} className="text-xs bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded font-bold transition shadow-sm">End Call</button>}
                  </div>
                  {videoUrl ? (
                    <div className="flex-1 bg-black rounded-lg overflow-hidden mb-4 h-64 border-2 border-purple-500"><iframe src={videoUrl} allow="camera; microphone; fullscreen" className="w-full h-full border-none" /></div>
                  ) : (
                    <div className="flex-1 bg-gray-50 border border-gray-200 rounded-lg p-4 overflow-y-auto space-y-3 mb-4 h-64">
                      {chatHistory.map((msg, idx) => (
                        <div key={idx} className={`flex flex-col ${msg.sender === 'Management' ? 'items-end' : 'items-start'}`}>
                          ...
                          <div className={`px-4 py-2 rounded-lg text-sm max-w-[85%] ${msg.sender === 'Management' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-200 text-gray-800 rounded-bl-none'}`}>{msg.text}</div>
                        </div>
                      ))}
                    </div>
                  )}
