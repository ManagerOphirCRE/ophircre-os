...
function getEmailBody(payload: any): string {
  ...
  } else encodedBody = payload.body?.data || '';
...
export async function GET(req: Request) {
  try {
    ...
    const userEmail = searchParams.get('userEmail'); // NEW: Strict user check
    if (!orgId || !userEmail) throw new Error("Missing credentials");

    // ONLY fetch tokens added by the specific user currently logged in
    const { data: tokens } = await supabase.from('google_tokens').select('*').eq('organization_id', orgId).eq('added_by_user', userEmail);
        ...
        if (gmailRes.data.messages) {
          for (const msg of gmailRes.data.messages) {
            ...
            await supabase.from('email_inbox').insert([{
              ...
              snippet: msgData.data.snippet, body_html: getEmailBody(msgData.data.payload),
              date: internalDate ? new Date(Number(internalDate)).toISOString() : new Date().toISOString()
            }]);
            totalSynced++;
          }
        }
