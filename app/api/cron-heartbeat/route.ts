...
export async function GET(req: Request) {
  try {
    ...
    // 2. Rent Escalations (Reminder)
    const { data: escalations } = await supabase.from('leases').select('*, tenants(name)').eq('next_escalation_date', target30);
    if (escalations && escalations.length > 0) {
      const tasks = escalations.map(l => ({ title: `ACTION REQUIRED: Rent Escalation for ${l.tenants?.name}`, description: `Rent is scheduled to escalate on ${l.next_escalation_date}. Review in Tenant Profile.`, status: 'To Do', tenant_id: l.tenant_id, organization_id: l.organization_id }));
      await supabase.from('tasks').insert(tasks);
      tasksCreated += tasks.length;
    }

    // 3. Tenant COIs expiring
    ...
    if (inventory && inventory.length > 0) {
      ...
      if (lowItems.length > 0) {
        const tasks = lowItems.map(item => ({ title: `RESTOCK: ${item.item_name}`, description: `Inventory low at ${item.properties?.name || 'Main Office'}. Only ${item.quantity} remaining.`, status: 'To Do', organization_id: item.organization_id }));
        await supabase.from('tasks').insert(tasks);
        tasksCreated += tasks.length;
      }
    }

    // 7. NEW: Preventative Maintenance Engine
    ...
    if (pmSchedules && pmSchedules.length > 0) {
      ...
      for (const pm of pmSchedules) {
        // Create the task
        pmTasks.push({
          title: `PREVENTATIVE: ${pm.title}`,
          description: pm.description || 'Routine maintenance required.',
