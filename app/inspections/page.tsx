  ...
  async function createInspection(e: any) {
    ...
    try {
      await supabase.from('inspections').insert([{
        tenant_id: tenantId,
        property_id: selectedTenant.leases[0].property_id,
        space_id: selectedTenant.leases[0].space_id,
  ...
  return (
    <>
      ...
      <main className="flex-1 overflow-y-auto p-8 bg-gray-100">
          ...
          {inspections.map(insp => (
            <div key={insp.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col">
              <div className="flex justify-between items-start mb-4 border-b pb-4">
                <div>
                  <h3 className="font-bold text-lg text-gray-900">{insp.tenants?.name}</h3>
                  <p className="text-sm text-gray-500">{insp.properties?.name} - {insp.spaces?.name}</p>
                </div>
                <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${insp.status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {insp.status}
                </span>
              </div>
              ...
              <button onClick={() => setActiveInspection(insp)} className="mt-auto w-full bg-gray-100 hover:bg-gray-200 text-gray-800 py-2 rounded font-bold transition">
                {insp.status === 'Completed' ? 'View Report' : 'Perform Inspection →'}
              </button>
            </div>
          ))}
          {inspections.length === 0 && <div className="col-span-full p-12 text-center text-gray-500 border-2 border-dashed border-gray-300 rounded-xl">No inspections scheduled.</div>}
        </div>
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg w-[400px]">
              ...
              <form onSubmit={createInspection} className="space-y-4">
                <div>
                  ...
                  <select required className="w-full border p-2 rounded outline-none" value={tenantId} onChange={(e) => setTenantId(e.target.value)}>
                ...
                <div>
                  ...
                  <select className="w-full border p-2 rounded outline-none" value={type} onChange={(e) => setType(e.target.value)}>
                    <option value="Move-In">Move-In</option>
                    <option value="Move-Out">Move-Out</option>
                    <option value="Annual Routine">Annual Routine</option>
                  </select>
                </div>
                <div className="flex justify-end space-x-3 pt-4 border-t">
                  ...
                  <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded font-medium">Create</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {/* PERFORM INSPECTION MODAL (The Digital Checklist) */}
        {activeInspection && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col">
              <div className="flex justify-between items-center border-b pb-4 mb-4">
                <h3 className="text-2xl font-bold">{activeInspection.inspection_type} Checklist</h3>
                <button onClick={() => setActiveInspection(null)} className="text-gray-400 hover:text-gray-800 text-2xl">&times;</button>
              </div>
              ...
              <div className="space-y-3 flex-1 overflow-y-auto mb-6">
                ...
                <div className="pt-4">
                  ...
                  <textarea className="w-full border p-3 rounded-lg outline-none h-24 resize-none" value={activeInspection.notes || ''} onChange={(e) => setActiveInspection({...activeInspection, notes: e.target.value})} placeholder="Add any damage notes here..." />
                </div>
              </div>

              <button onClick={saveInspection} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-bold transition shadow-sm">
