...
export default function TrashBinPage() {
  ...
  useEffect(() => { if (orgId) fetchTrash(); }, [orgId]);
  ...
  async function fetchTrash() {
    ...
    const { data: pData } = await supabase.from('properties').select('*').eq('is_deleted', true).order('name');
    if (pData) setDeletedProperties(pData);
    setIsLoading(false);
  }
  ...
  async function restoreItem(table: string, id: string) {
    ...
    try {
      ...
      fetchTrash();
    } catch (error: any) { alert("Error restoring item: " + error.message); }
  }
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-gray-100">
        {isLoading ? <p className="text-gray-500">Loading trash bin...</p> : (
          <div className="space-y-8 max-w-5xl mx-auto">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {deletedTenants.map(t => (
                      <tr key={t.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-right"><button onClick={() => restoreItem('tenants', t.id)} className="bg-green-600 text-white px-4 py-2 rounded font-bold text-xs hover:bg-green-700 transition">♻️ Restore</button></td>
                      </tr>
                    ))}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-4 bg-red-50 border-b border-red-100"><h3 className="font-bold text-red-800">Deleted Properties</h3></div>
              <div className="overflow-x-auto w-full">
                <table className="min-w-full divide-y divide-gray-200 whitespace-nowrap">
                  <tbody className="divide-y divide-gray-200">
                    {deletedProperties.map(p => (
                      <tr key={p.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-right"><button onClick={() => restoreItem('properties', p.id)} className="bg-green-600 text-white px-4 py-2 rounded font-bold text-xs hover:bg-green-700 transition">♻️ Restore</button></td>
                      </tr>
                    ))}
                    {deletedProperties.length === 0 && <tr><td colSpan={3} className="px-6 py-8 text-center text-gray-500">No deleted properties.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {deletedVendors.map(v => (
                      <tr key={v.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-right"><button onClick={() => restoreItem('vendors', v.id)} className="bg-green-600 text-white px-4 py-2 rounded font-bold text-xs hover:bg-green-700 transition">♻️ Restore</button></td>
                      </tr>
                    ))}
